...
class Layers(nn.Module):
    ...
    def forward(self, embed):
        embed = self.ly(embed)
        embed = self.norm(embed.permute(0, 2, 1)).permute(0, 2, 1)
...
class TransitionModel(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        for _ in range(4):
            self.block.append(
                Layers(in_channels=in_channels, out_channels=in_channels)
            )
        self.proj = nn.Linear(in_features=in_channels, out_features=out_channels, bias=False)

    def forward(self, embed):
        for idx, ly in enumerate(self.block):
            embed += ly(embed)
        out = self.proj(embed.permute(0, 2, 1))
        return out
...
class ProtT5Conv1dCRF4AAClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels):
        ...
        embeddings = self.transition(embeddings.permute(0, 2, 1))
    ...
    def inference(self, input_ids, attention_mask) -> List[dict]:
        embeddings = self.base_embedding(input_ids, attention_mask).last_hidden_state
        emissions = self.transition(embeddings.permute(0, 2, 1))
